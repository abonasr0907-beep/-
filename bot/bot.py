...
# توكن البوت — يتم تعيينه عبر المتغير البيئي BOT_TOKEN
...
async def _finalize_offer(update, uid, query=None):
    ...
    # المصدر الوحيد للعروض هو ملف الموقع
...
def _ai_response(text):
    """محرّك الردود الذكي للبوت"""
    t = text.lower()
    data = load_offers_json()
    offers = data.get("offers", [])

    if any(w in t for w in ["كم", "عدد", "احصائ", "إحصائ"]):
        farms = sum(1 for o in offers if o.get("type") == "farm")
        resthouses = sum(1 for o in offers if o.get("type") == "resthouse")
        lands = sum(1 for o in offers if o.get("type") == "land")
        return f"📊 لديك {len(offers)} عرض: {farms} مزرعة، {resthouses} استراحة، {lands} أرض سكنية."

    if "مزراع" in t or "مزرعة" in t:
        farm_offers = [o for o in offers if o.get("type") == "farm"]
        if not farm_offers:
            return "🌿 لا توجد مزارع منشورة حالياً."
        result = "🌿 المزارع المتاحة:\n\n"
        for o in farm_offers[:10]:
            result += f"• {o['id']} — {o.get('area','')} — {o.get('size_sqm','')} م² — {o.get('price_text','')}\n"
        return result

    if "استراح" in t:
        rh_offers = [o for o in offers if o.get("type") == "resthouse"]
        if not rh_offers:
            return "🏡 لا توجد استراحات منشورة حالياً."
        result = "🏡 الاستراحات المتاحة:\n\n"
        for o in rh_offers[:10]:
            result += f"• {o['id']} — {o.get('area','')} — {o.get('size_sqm','')} م² — {o.get('price_text','')}\n"
        return result

    if "أرض" in t or "ارض" in t or "اراضي" in t:
        land_offers = [o for o in offers if o.get("type") == "land"]
        if not land_offers:
            return "🗺️ لا توجد أراضٍ سكنية منشورة حالياً."
        result = "🗺️ الأراضي السكنية:\n\n"
        for o in land_offers[:10]:
            result += f"• {o['id']} — {o.get('area','')} — {o.get('size_sqm','')} م² — {o.get('price_text','')}\n"
        return result

    if "رحمانية" in t:
        area_offers = [o for o in offers if "رحمانية" in o.get("area", "")]
        return f"📍 عروض الرحمانية: {len(area_offers)} عرض."

    if "هياثم" in t:
        area_offers = [o for o in offers if "هياثم" in o.get("area", "")]
        return f"📍 عروض الهياثم: {len(area_offers)} عرض."

    if any(w in t for w in ["سعر", "اسعار", "بوصلة"]):
        return ("💰 أسعار البوصلة العقارية:\n\n"
                ...
                "الضبيعة: أرض 700 ريال/م² | مزرعة 100 ريال/م² | استراحة 280K-1M\n"
                "العفجة: أرض 650 ريال/م² | مزرعة 95 ريال/م² | استراحة 260K-950K")

    if any(w in t for w in ["كيف", "اضيف", "أضيف", "اضافة", "إضافة"]):
        return ("📝 لإضافة عرض جديد:\n\n"
                "1. اضغط «إضافة عرض جديد»\n"
                ...
                "6. أرسل المساحة\n"
                "7. أرسل السعر\n"
                "8. سيتم توليد النص التسويقي تلقائياً\n"
                "9. سيتم النشر مباشرة على الموقع ✅")

    if any(w in t for w in ["مساعدة", "help", "اوامر", "أوامر"]):
    ...
    return "🤖 لم أفهم سؤالك تماماً. جرّب:\n• «كم عدد العروض؟»\n• «اعرض المزارع»\n• «كم أسعار الهياثم؟»\n• «كيف أضيف عرضاً؟»"
